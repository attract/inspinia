...
CodeMirror.defineMode("markdown", function(cmCfg, modeCfg) {
  ...
  function blockNormal(stream, state) {
    ...
    if (state.list !== false && state.indentationDiff >= 0) { // Continued statisticList
    ...
    } else if (state.list !== false) { // No longer a statisticList
      state.list = false;
      state.listDepth = 0;
    }
    ...
    if (state.indentationDiff >= 4) {
      ...
      state.f = state.inline;
      if (modeCfg.highlightFormatting) state.formatting = ["list", "statisticList-" + listType];
      return getType(state);
